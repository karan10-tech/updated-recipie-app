...
export default function Home() {
  ...
  // Check if user is logged in and get userID safely
  const isLoggedIn = currentUser?.data?.data?.user?._id;
  ...
  useEffect(() => {
    ...
    const fetchSavedRecipes = async () => {
      // Only fetch saved recipes if user is logged in
      if (!userID) return;
  ...
  const saveRecipe = async (recipeID) => {
    ...
    if (!isLoggedIn) {
      message.warning("Please login to save recipes");
      return;
    }

    try {
      const response = await axios.put(`${API_BASE_URL}/api/v1/recipe/save`, {
        recipeID,
        userID,
      });
      setSavedRecipes(response.data.data.savedRecipes);
